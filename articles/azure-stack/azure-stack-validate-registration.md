...
2. From the PowerShell prompt, run the following to set `$registrationCredential` as the account that is the subscription owner. Replace `subscriptionowner@contoso.onmicrosoft.com` with your account and tenant: 
   ...
   ```
> [!NOTE]
  > As a CSP, when using an Shared Services or IUR subscription, you will need to provide the credentials of a user from that AAD. Usually this will be `subscriptionowner@iurcontoso.onmicrosoft.com`. That user needs to be have the appropriate credentials described above.
